...
#这个是RSC优化的一个点，生成一个权重矩阵
def generate_center_weighted_mask(H_feat: int, W_feat: int, device: torch.device, 
                                   border_width: int = 2, 
                                   center_weight: float = 0.1, 
                                   border_weight: float = 10, 
                                   highlight_left_border: bool = False,
                                   left_border_extra_weight: float = 0.5) -> torch.Tensor:
    """
    参数:
        H_feat (int): 特征图的高度。
        W_feat (int): 特征图的宽度。
        device (torch.device): 生成的 Tensor 所在的设备 (如 'cpu' 或 'cuda')。
        border_width (int): 边缘的宽度（像素/特征图单位），默认值为 1。
        center_weight (float): 中心区域的权重值，默认值为 0.1 (较低)。
        border_weight (float): 边缘区域的权重值，默认值为 1.0 (较高)。
        highlight_left_border (bool): 是否额外强调左侧边缘，默认值为 False。
        left_border_extra_weight (float): 如果强调左侧边缘，左侧边缘额外增加的权重值。
                                          最终左侧边缘权重为 border_weight + left_border_extra_weight。

    返回:
        torch.Tensor: 形状为 (1, 1, H_feat, W_feat) 的权重矩阵。
    """
    # 初始化矩阵，默认填充为中心权重
    W_spuriousness_map = torch.full((1, 1, H_feat, W_feat), center_weight, dtype=torch.float32, device=device)
    # 设置上、下、左、右边框的权重
    # 上边框
    W_spuriousness_map[:, :, :border_width, :] = border_weight
    # 下边框
    W_spuriousness_map[:, :, H_feat - border_width:, :] = border_weight
    # 左边框
    W_spuriousness_map[:, :, :, :border_width] = border_weight
    # 右边框
    W_spuriousness_map[:, :, :, W_feat - border_width:] = border_weight
    # 额外强调左边框 (如果需要)
    if highlight_left_border:
        W_spuriousness_map[:, :, :, :border_width] += left_border_extra_weight
        # 确保权重不超过一个合理的最大值，例如 2.0 或 3.0，防止过大导致梯度爆炸
        W_spuriousness_map = torch.clamp(W_spuriousness_map, 0.0, 3.0) 
    #print(W_spuriousness_map)
    return W_spuriousness_map
...
def compute_irm_penalty(processed_features_list, labels_list, classifier_head, device):
    ...
    Args:
        processed_features_list (list[Tensor]): 每个环境经过 RSC 处理后的特征列表。
        labels_list (list[Tensor]): 每个环境的标签列表。
        classifier_head (nn.Module): 模型的分类器部分 (nn.Sequential)。
        device (torch.device): 训练设备。
    """
    if len(processed_features_list) < 2:
        return torch.tensor(0.0, device=device)

    # 1. 创建一个与真实分类器权重维度相同的“虚拟”分类器
    #    这个虚拟分类器仅用于计算梯度，其权重是可求导的，但不参与主模型的优化。
    #    它模拟了 IRM 论文中对一个固定的虚拟分类器求梯度的思想。
    #    这里我们简单地使用一个 nn.Parameter 作为虚拟权重。
    #    LeNet5 的分类器输入是 16 * 5 * 5 = 400
